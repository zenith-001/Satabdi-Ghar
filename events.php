...
<body>
  ...
  <div class="nav">
    ...
    <div class="right">
      ...
      <form action="events.php" method="get" class="search">
  ...
  <!-- nav end   -->
  <p class="mytitle">
    Welcome to the events Page of Kushma Art Project
  </p>
  ...
  if (isset($_GET['search'])) {
    ...
    $result = mysqli_query($conn, "SELECT * FROM events_list where LOWER(Title) LIKE LOWER('%" . $_GET['search'] . "%')");
    ...
    if ($count >= 1) {
      ...
      for ($i = 0; $i < $count; $i++) {
        $c = $i + 1;
        $row = $array[$i];
        echo "<div class='card' onclick='location.href=`event.php?id=" . $row["ID"] . "`'>\n";
    ...
    $result = mysqli_query($conn, "SELECT * FROM `events_list`");
    while ($row = mysqli_fetch_array($result)) {
      $array[] = $row;
    }
    $count = count($array);
