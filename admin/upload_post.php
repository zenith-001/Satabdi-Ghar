...
if ($_SERVER['REQUEST_METHOD'] == "POST") {
    ...
    $blogdir = "../db_images/blog/$identity";
    if (!file_exists($blogdir)) {
        mkdir("../db_images/blog/" . $identity, 0777, true);
        mkdir("../db_images/blog/" . $identity."/res", 0777, true);
        chmod($blogdir, 0777);
    ...
    $sql = "INSERT into blogs_list (`Title`,`ID`,`Date`,`Description`,`Resources`,`Resources_Index`) VALUES ('$title','$identity','$date','$content','$targetDir','$totalFiles')";
...
<body>
    ...
    <h2 class="desc">Write content to be published as a post in the blogs page.</h2>
    <br>
    <form action="upload_post.php" method="POST" enctype="multipart/form-data">
        Title:<br>
        <input  maxlength="55" minlength="5" required placeholder="Title" type="text" name="title"><br><br>
        Thumbnail Image:<br>
        <input required id="file" type="file" name="images[]" accept="images/*" multiple><br><br>
        Content:<br>
        <input type="hidden" name="date">
        <input type="hidden" name="identity">
        <textarea minlength="10" maxlength="5000" required placeholder="Content" type="text"
