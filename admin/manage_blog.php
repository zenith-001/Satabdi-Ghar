...
<body>
    ...
    <h3 class="desc">This is the list of all the posts made till this date in the blogs page of the site</h3>
    <?php
    ...
    function deleteRecord($conn, $id)
    {
        ...
        $sql = "DELETE FROM blogs_list WHERE ID = $id";
        if (mysqli_query($conn, $sql)) {
            echo "Deleting...";
            $folder_path = "../db_images/blog/$id/res";
            chmod($folder_path, 0777);
            chmod("../db_images/blog/$id", 0777);
            // Delete the associated folder and its contents
            if (is_dir($folder_path)) {
                // Use recursive deletion to delete folder and its contents
            $success = rmdir("../db_images/blog/$id");
                if ($success) {
    ...
    if (isset($_GET['delete_id'])) {
        ...
        if (deleteRecord($conn, $delete_id)) {
            ...
            echo "<script>alert('Failed to delete record!!! ".exec('whoami')."');</script>";
        }
    }

    // Fetch all data from the database table
    $sql = "SELECT * FROM blogs_list";
